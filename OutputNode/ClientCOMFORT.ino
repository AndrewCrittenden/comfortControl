...
void setupCOMFORT() {
  ...
  pinMode(A0, OUTPUT);       //set analog 0 to output
  pinMode(A1, OUTPUT);       //set analog 1 to output
  pinMode(A2, INPUT);        //set analog 2 to input
  pinMode(A3, INPUT);        //set analog 3 to input
}

inline float calculateSeebeck(float Tinside, float Toutside)  {
  const float s1 = 0.0133450;      //initialize Seebeck constants
  const float s2 = -0.0000537574;
  const float s3 = 0.000000742731;
  const float s4 = -0.00000000127141;
  float SM;
...
inline float calculateResistance(float Tinside, float Toutside)  {
  const float r1 = 2.08317;        //initialize resistance constants
  const float r2 = -0.0198763;
  const float r3 = 0.0000853832;
  const float r4 = -0.00000009031343;
  float RM;
  float DT = Toutside - Tinside;
...
inline float calculateThermalConductance(float Tinside, float Toutside)  {
  const float k1 = 0.476218;       //initialize thermal conductance constants
  const float k2 = -0.00000389821;
  const float k3 = -0.00000864864;
  const float k4 = 0.0000000220869;
  float KM;
  float DT = Toutside - Tinside;
...
void onDataReceived(responseData inData) {
  // Code for what to do with incoming data here
   ////////////////////////////////////////////////////module condition calculation
  const float A_1 = 0.003354016;    //initialize constants for thermistor equation
  const float B_1 = 0.000300131;
  const float C_1 = 0.00000508516;
  const float D_1 = 0.000000218765;
  
  float Vinside = 3.3 - analogRead(A3)*3.3/4095;     //store fridge side voltage
  float Voutside = 3.3 - analogRead(A2)*3.3/4095;          

  float Rinside = (9860*Vinside)/(3.3-Vinside);  //Calculate the resistance of the fridge-side thermistor
  float Routside = (9810*Voutside)/(3.3-Voutside);        

  float Tinside = A_1 + B_1*log(Rinside/10000) + C_1*pow(log(Rinside/10000), 2) + D_1*pow(log(Rinside/10000), 3);     //Calculate the fridge-side temp using thermistor response
  Tinside = (1/Tinside);     //convert temp to kelvin
  float Toutside = A_1 + B_1*log(Routside/10000) + C_1*pow(log(Routside/10000), 2) + D_1*pow(log(Routside/10000), 3);             
  Toutside = (1/Toutside); 
...
///////////////////////////////////////////////////////////////
  ...
  if(abs(Hdeli) < abs(inData.powerIn * diff) || (Hdeli > 0 ^ inData.powerIn > 0) && abs(inData.powerIn) > 0.0001) { //output is needed
    ...
    if(inData.powerIn < 0)  {     //if cooling
      ...
      if(Imin > Iprime) {
        ...
        Iout = 4;   //(-b+sqrt(pow(b, 2)-(4*a*c)))/(RM);
      }
  }
  ...
  if(Iout < 0) {
    ...
    Vgs = 3.6 + sqrt(Iout / 0.34);      //find voltage needed across the device

    Vout = Vgs / 2.19;                      //find the voltage needed by microcontroller


    if (Vout > 3.3)   {            //limit outputs above the microcontroller's capability
        Vout = 3.3;
    }

    else if (Vout < 1.4)  {            //ensure output does not drop below OP amp voltage swing
        Vout = 1.4;
    }
    T0 = T1;

  analogWrite(A1, 1.4*4095/3.3);
  analogWrite(A0, Vmode*4095/3.3);
  analogWrite(A1, Vout*4095/3.3);
  
  Serial.print("Power Input: ");
  Serial.print(inData.powerIn);
  Serial.print(", Ambient Temp: ");
  Serial.println(inData.tempIn);
}
