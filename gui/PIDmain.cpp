...
int PIDmain() {
    //TODO find out why cout is not printing to terminal, maybe use qDebug or printf instead
    qDebug() << "Starting PID";
    // Retrieve values from GUI see measurements.h for all global variables
    qDebug() << g_isComfortable;
    qDebug() << g_setpoint_temperature;
    // Set values to send to the GUI.
    g_heatCoolOutput = 500; //Set the heating/cooling value to send to the GUI
